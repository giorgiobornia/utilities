!Questa e' la versione 'fast' del codice: per ottimizzare i tempi di calcolo tutte le stampe che possono far perdere tempo sono state commentate con '!!!!'. Scommentare se si desidera.
...
!inizializzo tutti i contatori... NB: se chiami una subroutine dentro un ciclo do, tipicamente le inizializzazioni vanno fuori dalla subroutine
          maxncoll = 0
          ntprimo = 0
          nt = 0
          nr = 0
...
!     allocate(ncoll(nstorie - nstart + 1))
...
!in questo caso direi che per ottimizzare il codice bisognerebbe iniziare dai coseni circa 0, perche' se devia piano piano e' piu' facile che esca a 95 gradi anziche' a 180... in qualche mooodo...
...
!memorizzo in una variabile dinamica il numero di collisioni totali di ogni particella
!   ncoll(z - nstart + 1) = j
...
!---conversione a partire dalle w_i (percentuali in massa) dei composti nelle N_j [#/barn*cm] degli elementi
 Vtot = 0.d0
  ...
  do j = 1,5
     wn(j)=((pcomp(j)/mmol(j))*Avo)/(Vtot*1.0d30) !CONTROLLARE N particelle per unit di volume della sostanza j-esima
  enddo
...
!-----modo equivalente per scrivere questi open con un doppio ciclo
!character*15 interact(3)
...
!   do l = 1, 6
!       do m = 1, 3
!      open(20 + 3*(l-1) + m-1, FILE = './input///nomeel(l)//_//interact(m)//.dat',STATUS = 'OLD')
!       end do
!  end do
...
!  integer ncoll(*)    !AAA, l'importante e' che gli passi l'indirizzo della prima casella di memoria
  integer maxncoll
  logical analytic


!cerco il massimo degli ncoll!ora non mi serve piu' calcolarlo qui, glielo passo direttamente con maxncoll
!    maxncoll = ncoll(1)
!        do j = 2, (nstorie - nstart + 1)
!         if (ncoll(j) .gt. maxncoll) then
!         maxncoll = ncoll(j)
!        end if
!    end do
...
!le due successive stampe servono all'utente per capire (in qualche mooodo...) se proseguire con il numero di storie e di quanto aumentare il limite massimo alle collisioni 
  ...
  if (analytic .eqv. .false.) then
  ct = REAL(nt + ntprimo) / REAL(nstorie - nccum)  !NB: nel caso non analitico quelle troncate non le devo contare perche' non spariscono dal fascio per motivi fisici ma artatamente ad opera dell'utente
...
!le tabelle mi hanno fatto penare!! la sezione d'urto di assorbimento e' per certe energie IDENTICAMENTE NULLA!! le particelle trasmesse andavano bene, ma mi dava errore per quelle assorbite

!ma fortran 90 ha 132 righe di limite?!?

!possibilita':
!qual e' la composizione in massa, per dato s e data energia del fascio, che MINIMIZZA la trasmittanza?
!e' sempre detto che aumentando lo spessore diminuisce la trasmittanza, o c'e' un intervallo di spessori per cui si ha un build-up (=> trasmittanza NON monotona decrescente con lo spessore)?
!per dato spessore e data composizione, qual e' la massima energia che mi posso permettere per avere una trasmittanza minore di un valore massimo? si' ma uno non costruisce una macchina radiogena in funzione dello schermo, costruisce lo schermo in funzione della macchina...
!quindi preferisco ragionare a macchina data ( => energia dei neutroni data) e schermo variabile
!se uno volesse generalizzare al caso di un fascio non monocromatico ne' monodirezionale, dovrebbe dare in pasto uno spettro in ingresso misurato con vari angoli e varie energie... ma se uno ha misurato uno spettro in ingresso bastava che si mettesse dall'altra parte e misurava anche lo spettro di uscita, quindi non avrebbe senso fare dei conti, a meno che uno non voglia fare una procedura di validazione sperimentale del codice

!una cosa che si potrebbe fare e' arrivare in fondo con un numero di particelle che ha colliso e uno che non ha colliso, poi chiedere all'utente:
 !quante particelle vuoi che facciano la loro storia in tutto?
  ! poi uno prende quelle che ha gia', non le butta e continua fino ad arrivare al numero totale di storie desiderato; ovviamente in questo si chiede all'utente di cambiare il numero di collisioni, e l'utente lo fa consapevolmente, guardando i dati di output, in particolare non il numero massimo di collisioni che e' evidentemente quello da noi imposto e che pone un freno, bensi' il numero di particelle troncate e anche quante di queste avrebbero ancora interagito dentro lo schermo.

!La richiesta di un confronto analitico riguarda solo il montecarlo, quindi potrei inserirla all'interno della subroutine montecarlo piuttosto che nella subroutine di input, nella quale considero solo i dati FISICI(energia dei neutroni e composizione in massa dello schermo) e GEOMETRICI (spessore della lastra). Le richieste di tipo statistico (numero di particelle e numero massimo di collisioni per ciascuna particella) le sposto nella subroutine montecarlo.
!Quindi ho suddiviso richieste di tipo: fisico, geometrico e statistico. 





!poi come si potrebbe fare per RIESUMARE le storie troncate? bisognerebbe tenere traccia di ciascuna, registrarne gli ultimi dati e poi riprenderle...

!allora: parto con 1000 storie supponiamo; di queste 130 vengono troncate... allora io dico: finiscimi quelle 130; oppure posso anche dirgli: non solo finiscimi quelle 130 ma anche fammene altre 200... se poi anche qui avro' delle collise allora poi 

! il fatto e' che io conservo le storie concluse ma non riesumo quelle troncate: l'ottimizzazione sarebbe di riesumare quelle troncate
